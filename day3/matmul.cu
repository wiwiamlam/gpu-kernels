...
__global__ void gpu_matmul(int *A, int *B, int *C, int M, int K, int N) {
	...
	// printf("x: %d y: %d\n", x, y);
	// add boundary as the CUDA block size might be larger than the actual data block
	// which cause data access to C out of bound (illegal access)
	if (y < M && x < N) {
		int result = 0;
		for (int k = 0; k < K; k++) {
			result += A[K*y + k] * B[N*k + x];
		}
		C[N*y + x] = result;
	}
	return;
...
int main(int argc, char **argv) {
	...
	dim3 numOfBlocks((m + threadsPerBlock.x - 1)/threadsPerBlock.x, (n + threadsPerBlock.y - 1)/threadsPerBlock.y);

	// declare events for timing kernel
	...
	cudaEventElapsedTime(&gpu_milliseconds, start, end);
